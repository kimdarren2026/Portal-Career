...
| Stage | Control |
| --- | --- |
| **Accept** | **Content-inspected MIME**, never the client-declared header and never the extension, both of which are advisory only. Size ceiling enforced at the application (authoritative) with transport ceilings configured **above** it. **Candidate documents (frozen 25 August 2026):** allowlist is **`application/pdf` only**, admitted when the inspected type is `application/pdf` **and** the content begins with `%PDF-`; maximum **10 MiB / 10,485,760 bytes** as a **single global limit**, not varying by `document_type`; upload rate limit **20 per hour per candidate**. DOC, DOCX, XLS, XLSX, ZIP, RAR, 7Z, SVG, images, audio, and video are **not approved** — widening requires a new security and business review. Company legal documents keep their own FR-ONB-002 rules |
...
| R-3 | Malware scanning is conditional in FSD ("if available") | Accepted risk if no scanner is provisioned. Quarantine-and-promote design means a scanner can be added later without redesign. **Candidate upload is deliberately narrowed to `application/pdf`, delivered attachment-only and never rendered in a trusted origin, so the PDF-only MVP is acceptable without a scanner. A scanner is a prerequisite for admitting any macro-capable or container format** |
