...
**Amendment:** Campus Recruitment scope activation, 1 September 2026 — approved Product Owner / SPEC-DOC decision (Part X item 60). The previously-DEFERRED `CAMPUS_SCOPE` / `HR_ADMIN` (Admin Kepegawaian) runtime is activated for the Karier di Kampus track that BRD/FSD already require (FSD §5.5, FR-HR-001..007, §8.4). `POST /hr/vacancies` and the campus lifecycle (`publish` / `schedule` / `close` / `suspend` / `restore`, FSD §8.4) are implemented; `CAMPUS_SCOPE` is activated across applicant management, recruitment stages, selection schedule, evaluation, offering and recruitment outcome, each on `ownership_type = CAMPUS` vacancies only. Campus vacancies are `IN_PORTAL`-only, carry no company-verification gate, and are never moderated (no Career Center involvement). Public discovery gains a campus branch; `COMPANY_SCOPE` and PD-1's company predicate are byte-identical and unchanged. **This is a scope/runtime-activation decision — no new recruitment track, no new schema, no new application entity (campus applications remain `INTERNAL_APPLICATION` on the one shared candidate/application lifecycle), and no change to any company or Career Center authorization.** The activation itself is recorded here as a PO / SPEC-DOC decision, not attributed to BRD/FSD.
**Amendment:** In-app notification web surface, 1 September 2026 — approved Product Owner / SPEC-DOC decision (Part X item 59). The three notification operations — `GET /notifications`, `POST /notifications/{notification}/read`, `POST /notifications/read-all` — are reclassified from `VERSIONED_API` to `INERTIA_WEB` for the MVP browser portal (Laravel session guard + CSRF on the two mutations), the same pattern SPEC-DOC-05/-07/-08, SS-9 and OF-2 already established. Their `/api/v1` bearer-token twins remain reserved. `OWN` authorization by `user_id` is unchanged; company membership never widens the set. **No notification business rule, no domain notifier, and no `email_outbox` semantic changes.** BRD v1.1 already scopes in-app notifications into MVP and FSD already carries the `notifications` entity — this is a transport classification only, not attributed to BRD/FSD.
**Amendment:** Production Gap Closure V1 (PGC-V1) — approved Product Owner decisions PD-A…PD-G (Part X items 62–68; full text in `docs/decisions/PRODUCT_OWNER_DECISIONS.md`), recorded with batch `p1-production-blockers-batch-1b`. Activates application-document snapshot download (supersedes item 22 / RA-3 for the download operation), transactional email delivery (supersedes the `RUNBOOK.md` B-5 note), public vacancy reporting (Laporkan Lowongan), the company legal-document runtime with an MVP file/type policy (closes item 2), the ratified `APPLICATION_CONSENT_2026_08` canonical text, the Super Admin user directory + `ACTIVE ↔ SUSPENDED` account lifecycle, and a real minimal public homepage. BRD/FSD are unedited; these are Product Owner decisions recorded here.

**Amendment:** Super Admin vacancy-type reference scope, 1 September 2026 — approved Product Owner decision `SUPER_ADMIN_VACANCY_TYPE_REFERENCE_MVP` (Part X item 61). For MVP the Super Admin "Jenis Lowongan" navigation item is a **read-only reference surface** over the already-frozen `App\Domains\Vacancy\Enums\VacancyType` vocabulary (`CAMPUS_EMPLOYMENT`, `COMPANY_EMPLOYMENT`, `INTERNSHIP` — FR-VAC-001, INV-018), `SUPER_ADMIN` only. No create / rename / delete / disable / reorder; **no `vacancy_types` table, no schema conversion, no migration, no enum mutation.** Dynamic / configurable vacancy types are explicitly **DEFERRED beyond MVP**. This resolves the Super Admin navigation ambiguity only — it does not modify FR-VAC-001, INV-018, the enum, ownership rules, vacancy authoring, Public Discovery, `TargetAudience`, or `employment_type`. Not attributed to BRD/FSD.
...
**Surface:** `INERTIA_WEB`  ·  **MVP browser routes:** `GET /notifications` (JSON, `data.{items,pagination}` + `meta.unread_count`) and its Inertia realization `GET /notifikasi`

> **Reclassified for the MVP browser portal — approved Product Owner / SPEC-DOC decision (Part X item 59).** The three notification operations (`GET /notifications`, `POST /notifications/{notification}/read`, `POST /notifications/read-all`) are served over the **Laravel session guard with CSRF protection on the two mutations**, the same SPEC-DOC-05/-07/-08/SS-9/OF-2 pattern — not a Sanctum bearer token. The headings remain the canonical operation identifiers and the reserved `/api/v1` twins for when token authentication is activated for a non-browser client; promotion is then additive and requires no behavioural change. **`OWN` authorization and every business rule below are unchanged**, and no notifier or `email_outbox` semantic changes. BRD v1.1 already places in-app notifications inside MVP scope and FSD's conceptual model already carries the `notifications` entity — this decision is a transport classification, **not** attributed to BRD/FSD.
...
| 2 | ~~**Minimum company legal documents per organization type**~~ | **CLOSED — Product Owner decision PGC-V1 / PD-D** (`docs/decisions/PRODUCT_OWNER_DECISIONS.md`). No per-organization-type mandatory matrix for MVP: at least **one active** legal document of an allowed type (`NIB`, `AKTA_PENDIRIAN`, `SK_KEMENKUMHAM`, `IZIN_OPERASIONAL`, `DOKUMEN_LEGALITAS_LAINNYA`) is required before submit (INV-030), no individual type mandatory. Enforced by request/runtime validation only — no `CHECK`, no migration on `company_documents.document_type`. File policy: MIME `application/pdf` / `image/jpeg` / `image/png`, 10 MiB max, server-inspected content. No expiry-blocking rule for MVP. See PD-D for the full runtime |
...
| 22 | ~~**RA-3 — recruiter application-document download**~~ | **SUPERSEDED for the download operation — Product Owner decision PGC-V1 / PD-A** (`docs/decisions/PRODUCT_OWNER_DECISIONS.md`), approved with batch `p1-production-blockers-batch-1b`. The previously-deferred `GET /api/v1/application-documents/{applicationDocument}/download` (browser route `GET /application-documents/{applicationDocument}/download`) is **activated**. It streams the **immutable `application_documents` snapshot** (`snapshot_storage_reference` / `snapshot_name`) — never the candidate's live document — application-mediated, private-disk only, storage reference never exposed, no public/pre-signed URL, `Content-Disposition: attachment`, authorization before object retrieval, enumeration-safe `404`, safe `404` on a missing object, every download and every denied attempt audited as `document_access`. Authorization is `AUTHORIZATION_MATRIX.md` §4.6 exactly: `CANDIDATE` OWN, `COMPANY_RECRUITER`/`COMPANY_ADMIN` `COMPANY_SCOPE`, `HR_ADMIN` `CAMPUS_SCOPE`, `SELECTOR` active `ASSIGNED_STAGE`, `SUPER_ADMIN` ALLOW, `CAREER_CENTER` and `AUDITOR` DENIED. Metadata-only exposure on the detail read (the original RA-3 behaviour) is unchanged; `snapshot_storage_reference` / `snapshot_checksum` are still never returned in a JSON body |
...
| 53 | ~~**FE-7 — workplace mode v1 display (vocabulary closed)**~~ | **CLOSED — approved 29 August 2026.** Completes FE-5 (row 51): the v1 `vacancies.workplace_mode` display vocabulary is now closed for exactly `ONSITE` → "On-site", `HYBRID` → "Hybrid", `REMOTE` → "Remote". Frontend display only — stored values, request/filter query values, and the (still absent) DB CHECK are unchanged; no migration, no backend enum. No additional `workplace_mode` value is approved; a future/unknown value still falls through to the existing frontend unknown-value fallback (raw string), never a manufactured label. Product Owner decision recorded here, not attributed to BRD/FSD |
| 54 | ~~**FE-8 — candidate-visible stage fallback**~~ | **CLOSED — approved 29 August 2026.** Freezes the candidate presentation of a candidate-**visible** `STAGE_CHANGED` history row (completes FE-6 row 52, which covered the recruiter side): when `to_stage.candidate_visible_label` **is not null**, display it **verbatim**; when it **is null**, display the bare noun **"Tahap Seleksi"**. Never display the internal stage `name`, the raw `STAGE_CHANGED` enum, `actor_user_id`, the internal `reason`, or any audit payload. Display only — creates no application status, changes no stage-movement semantics, no `candidate_visibility` rule, no stage configuration, and no database semantics. An `INTERNAL` `STAGE_CHANGED` stays absent from the candidate payload (`ApplicationPresenter`'s `candidate_visibility = VISIBLE` filter remains sole authority). The recruiter timeline is unchanged and still shows "Tahap Seleksi Diubah" per FE-6. Product Owner decision recorded here, not attributed to BRD/FSD. Already implemented at `bc3bbb9` (`ApplicationPresenter::historyEvent` `stage_label`; `candidateHistoryLabel` in `lib/labels.ts`) — this row formalizes that behaviour |
| 55 | ~~**FE-9 — employment type authoring vocabulary (recruiter create/edit)**~~ | **CLOSED — approved 1 September 2026.** Completes FE-4 (row 50), which closed only `FULL_TIME`. The recruiter company-vacancy create/edit form offers exactly five `vacancies.employment_type` options and submits the raw code: `FULL_TIME` → "Penuh Waktu", `PART_TIME` → "Paruh Waktu", `CONTRACT` → "Kontrak", `FREELANCE` → "Freelance", `TEMPORARY` → "Sementara". Free text is **not** approved as an authoring input. This governs the authoring control and the frontend display map only — **no `CHECK` constraint, no migration, no backend `Rule::in` change, and no `employment_type` enum** are introduced; `VacancyFormRequest` still validates `string|max:64`. An unknown legacy stored value still falls through to the frontend raw-string fallback, never a manufactured label. Product Owner decision recorded here, not attributed to BRD/FSD |
| 56 | ~~**FE-10 — company verification status display vocabulary**~~ | **CLOSED — approved 1 September 2026.** Frontend display labels for `companies.verification_status` (already the fixed `App\Domains\Company\Enums\CompanyStatus` enumeration): `DRAFT` → "Draf", `PENDING_VERIFICATION` → "Menunggu Verifikasi", `REVISION_REQUIRED` → "Perlu Perbaikan", `VERIFIED` → "Terverifikasi", `REJECTED` → "Ditolak", `SUSPENDED` → "Ditangguhkan". Display only — stored enum values, `chk_companies_verification_status`, and the enum class are unchanged. "Terverifikasi" is a verification state and carries **no** Mitra Kampus / partnership meaning (PD-1 principle; `verification_status` and partnership are independent facts). BRD/FSD did not define these labels; Product Owner decision recorded here, not attributed to BRD/FSD |
| 57 | ~~**FE-11 — vacancy status display vocabulary**~~ | **CLOSED — approved 1 September 2026.** Frontend display labels for `vacancies.current_status` (already the fixed `App\Domains\Vacancy\Enums\VacancyStatus` enumeration): `DRAFT` → "Draf", `PENDING_REVIEW` → "Menunggu Tinjauan", `REVISION_REQUIRED` → "Perlu Perbaikan", `APPROVED` → "Disetujui", `SCHEDULED` → "Terjadwal", `PUBLISHED` → "Dipublikasikan", `REJECTED` → "Ditolak", `CLOSED` → "Ditutup", `EXPIRED` → "Kedaluwarsa", `SUSPENDED` → "Ditangguhkan". Display only — stored enum values, `chk_vacancies_current_status`, and the enum class are unchanged; `SUBMITTED`/`DIAJUKAN` is deliberately absent because submit is an action, never a status (FR-VAC-004, INV-004). BRD/FSD did not define these labels; Product Owner decision recorded here, not attributed to BRD/FSD |
| 58 | ~~**FE-12 — vacancy target audience display vocabulary**~~ | **CLOSED — approved 1 September 2026.** Frontend display labels for `vacancies.target_audience` (already the fixed `App\Domains\Vacancy\Enums\TargetAudience` enumeration, INV-006): `PUBLIC` → "Publik", `ALUMNI_ONLY` → "Alumni", `FINAL_YEAR_AND_ALUMNI` → "Mahasiswa Tingkat Akhir & Alumni", `INTERNAL` → "Internal". Display only and used as the recruiter authoring select's option labels — stored enum values, `chk_vacancies_target_audience`, `VacancyFormRequest`'s `VACANCY_TARGET_AUDIENCE_INVALID` rule, and the enum class are unchanged. No cross-field authoring restriction is introduced. BRD/FSD did not define these labels; Product Owner decision recorded here, not attributed to BRD/FSD |
| 60 | ~~**CAMPUS-1 / SPEC-DOC-10 — Campus Recruitment scope activation**~~ | **CLOSED — approved 1 September 2026.** The previously-DEFERRED `CAMPUS_SCOPE` / `HR_ADMIN` runtime is **activated**. This row **supersedes every "`CAMPUS_SCOPE` (`HR_ADMIN`) is not activated — no Campus vacancy runtime exists" / "inert, no Campus runtime" note** scattered through Parts VI–VIII of this document. What is now implemented: (a) `POST /hr/vacancies` — create a `CAMPUS_EMPLOYMENT` vacancy (`ownership_type = CAMPUS`, `organizational_unit_id` required + active, `company_id` NULL, `application_method` forced `IN_PORTAL`, **no company-verification gate**, **never moderated** — `PENDING_REVIEW`/`REVISION_REQUIRED`/`APPROVED`/`REJECTED` unreachable, INV-018/chk_vacancies_campus_status). (b) The campus lifecycle `POST /hr/vacancies/{vacancy}/{publish|schedule|close|suspend|restore}` (FSD §8.4): `DRAFT/SCHEDULED → PUBLISHED`, `DRAFT → SCHEDULED`, `PUBLISHED → CLOSED`, `PUBLISHED → SUSPENDED`, `SUSPENDED → PUBLISHED` (or `→ CLOSED` past `close_at`, mirroring B-2). `published_at` set once (INV-013). The scheduler now also drives campus `SCHEDULED → PUBLISHED` (open date) and `PUBLISHED → EXPIRED` (close date) with identical semantics (status + audit only, no notification). (c) Campus vacancy **read/edit/stages/screening reuse the shared `/vacancies/*` surface** — `VacancyScope` and `VacancyPolicy` carry a campus branch (HR_ADMIN, no membership). (d) `CAMPUS_SCOPE` branch added to `RecruiterApplicationScope`, `SelectionScheduleScope`, `EvaluationScope`, `OfferScope`, `RecruitmentOutcomeScope` and the H-5 incomplete query — HR_ADMIN reaches applications/schedules/evaluations/offers/outcomes on `ownership_type = CAMPUS` vacancies only, never a company vacancy. `RA-2` (`ApplicationProcessingGate`) reduces to the vacancy-status check for campus (no company component). (e) Public discovery: `PublicVacancyScope` gains a campus branch (`ownership_type = CAMPUS`, PUBLISHED, in-window, `target_audience != INTERNAL`, **no company gate**). The PD-1 company predicate is byte-identical and unchanged. **No new schema, no migration, no new `source_type`** — campus applications are `INTERNAL_APPLICATION` on the one shared candidate/application/stage/schedule/evaluation/offer/outcome lifecycle. **No company or Career Center authorization changed**: Career Center stays candidate-selection `DENY` and cannot see or moderate a campus vacancy (enumeration-safe 404); a company recruiter cannot author, publish, read or process anything campus; `HR_ADMIN` cannot reach the company track. Reporting (FR-REP-003 / `GET /reports`) remains **DEFERRED** — not part of this activation. The activation is recorded here as a PO / SPEC-DOC decision, not attributed to BRD/FSD |
| 59 | ~~**NOTIF-1 / SPEC-DOC-09 — in-app notification web surface**~~ | **CLOSED — approved 1 September 2026.** The three already-frozen notification operations — `GET /notifications`, `POST /notifications/{notification}/read`, `POST /notifications/read-all` — are reclassified `VERSIONED_API → INERTIA_WEB` for the MVP browser portal (Laravel session guard, CSRF on the two mutations), the same SPEC-DOC-05/-07/-08/SS-9/OF-2 pattern; the `/api/v1` twins stay reserved and inactive, no Sanctum/PAT activation. **Transport classification only.** `OWN` authorization by `notifications.user_id` is unchanged and company membership never widens it — a Company Admin does not see a member's notifications, two recruiters in one company never see each other's. `notifications` stays independent of `email_outbox`: no `PENDING`/`PROCESSING`/`FAILED`/`RETRY_SCHEDULED`/`DEAD_LETTER` transport state, attempt count, schedule or delivery error is ever exposed on this surface. The presenter returns exactly `id`, `type`, `title`, `body_reference`, `related_object_type` (stable logical name, INV-033), `related_object_id`, `read_at`, `created_at`. `type`/`title` are opaque stable codes emitted by the existing domain notifiers — **no notification-type vocabulary, and no Indonesian label per type, is closed by this decision**; the frontend displays the stored `title` with cosmetic formatting only and an unknown code renders identically. No domain notifier, no outbox writer, no schema, and no migration is touched. BRD v1.1 already scopes in-app notifications into MVP and FSD already carries the `notifications` entity; this row records the **transport decision only**, not attributed to BRD/FSD. `read-all` and mark-one carry no audit (not an FR-AUD-001 event) and no idempotency header (mark-one is naturally idempotent) |
| 61 | ~~**SUPER_ADMIN_VACANCY_TYPE_REFERENCE_MVP — Super Admin "Jenis Lowongan" scope**~~ | **CLOSED — approved 1 September 2026.** For MVP the Super Admin navigation item "Jenis Lowongan" is a **read-only reference surface** over the already-frozen `App\Domains\Vacancy\Enums\VacancyType` vocabulary — exactly `CAMPUS_EMPLOYMENT`, `COMPANY_EMPLOYMENT`, `INTERNSHIP` (FR-VAC-001, `chk_vacancies_vacancy_type`, INV-018). It renders per type the raw code, the frozen FE-3 display label (row 49), and the source-backed ownership classification (`CAMPUS_EMPLOYMENT` → campus-owned per INV-018; `COMPANY_EMPLOYMENT` / `INTERNSHIP` → company-owned per `VacancyType::companyAuthorable()`). `SUPER_ADMIN` only — no global-reader branch; Auditor and every other persona are denied. **No Super Admin operation may create a fourth type, rename a raw code, delete a type, disable a type, or reorder business semantics.** No `vacancy_types` table, no schema conversion, no migration, no `CHECK` change, no enum mutation, no configurable custom types. Dynamic / configurable vacancy types are explicitly **DEFERRED beyond MVP**. This is a reference/read administration surface — **not CRUD**. It does not replace or modify FR-VAC-001, INV-018, the `VacancyType` enum, campus-vs-company ownership rules, vacancy authoring policies, Public Discovery, `TargetAudience`, or `employment_type`; it resolves the Super Admin navigation ambiguity only. BRD/FSD are unedited; Product Owner decision recorded here, not attributed to BRD/FSD |

| 62 | ~~**PGC-V1 / PD-A — application document (snapshot) download**~~ | **CLOSED — Product Owner decision PGC-V1** (`docs/decisions/PRODUCT_OWNER_DECISIONS.md`). Supersedes item 22 for the download operation — see that row |
| 63 | ~~**PGC-V1 / PD-B — transactional email delivery**~~ | **CLOSED — Product Owner decision PGC-V1.** Minimal factual version-controlled transactional templates approved for production (not marketing); every runtime `template_reference` has a deterministic renderer proven by a coverage check; standard STIKES Advaita Medika Tabanan / Portal Karir layout. Delivery runtime on the frozen `email_outbox` states: `PENDING → PROCESSING → SENT`, failure `→ FAILED_RETRYABLE` with backoff/retry, `→ DEAD_LETTER` at configured `max_attempts`. Sanitized `last_error_summary` (no credential/host/DSN/stack — INV-015/INV-035); concurrency- and duplicate-safe; SMTP failure never rolls back the business transaction; runtime `smtp_configurations` used when active else deployment `MAIL_*`; Super Admin requeue. Supersedes the `B-5` note in `RUNBOOK.md` §1/§9 |
| 64 | ~~**PGC-V1 / PD-C — Laporkan Lowongan (public vacancy reporting)**~~ | **CLOSED — Product Owner decision PGC-V1.** New `vacancy_reports` entity. `GET`/`POST /lowongan/{vacancy}/laporkan` (public slug). Both anonymous and authenticated reporters; anonymous never forced to register; authenticated identity auto-attached, reporter identity never public. Closed reason vocabulary `FRAUD_OR_SCAM` / `MISLEADING_INFORMATION` / `INAPPROPRIATE_CONTENT` / `INVALID_OR_EXPIRED_VACANCY` / `SUSPICIOUS_EXTERNAL_LINK` / `OTHER`. `reason` required, `details` optional except required for `OTHER` (max 2000). Lifecycle `NEW → UNDER_REVIEW → ACTIONED | DISMISSED`, Career Center owns transitions (no recruiter review; conflict-of-interest bar on a reviewer who filed the report; Super Admin read-only). Anti-spam: 5/IP/hour anonymous, 10/account/day authenticated; CSRF applies. Audit `vacancy_report_created` / `_review_started` / `_actioned` / `_dismissed`. Indefinite retention |
| 65 | ~~**PGC-V1 / PD-D — company legal document policy & runtime**~~ | **CLOSED — Product Owner decision PGC-V1.** Also closes item 2 — see that row. Activates the frozen `POST/GET/DELETE /companies/{company}/documents`, `.../{document}/download`, `.../{document}/supersede` runtime |
| 66 | ~~**PGC-V1 / PD-E — application consent canonical text**~~ | **CLOSED — Product Owner decision PGC-V1.** `APPLICATION_CONSENT_2026_08` ratified with the exact Indonesian text in `PRODUCT_OWNER_DECISIONS.md`. `consent_text_hash_reference` derived server-side from the canonical text + version; client hash never authoritative; any wording change requires a new version, historical versions never altered; INV-011 / INV-023 receiver/purpose/timestamp invariants unchanged. Completes/extends FE-1 (item 48) — the FE-1 copy is a shorter frontend paragraph; PD-E is the authoritative full consent statement bound to the hash |
| 67 | ~~**PGC-V1 / PD-F — Super Admin user directory & account suspension**~~ | **CLOSED — Product Owner decision PGC-V1.** `GET /admin/users` (`SUPER_ADMIN` only) returns `id` / `name` / `email` / `status` / `active_roles` / `created_at`, never a secret; text/status/role filters; pagination default 25 max 100; `created_at DESC`. Account lifecycle MVP `ACTIVE ↔ SUSPENDED` only (`DISABLED` deferred): `POST /admin/users/{user}/suspend` (reason required, ≤1000) / `restore` (reason optional), `SUPER_ADMIN` only. Suspension terminates active sessions immediately (OL-10 per-request check + server-side session rows cleared), does **not** notify the user (silent containment), audits `user_suspended` / `user_restored` with the reason and no secret |
| 68 | ~~**PGC-V1 / PD-G — public homepage**~~ | **CLOSED — Product Owner decision PGC-V1.** `/` renders a real minimal production homepage (fixed hero copy, two quick pathways to filtered `/lowongan`, up to 6 latest real `PUBLISHED` vacancies with a truthful empty state, auth CTAs, a Laporkan Lowongan entry). No fabricated statistics/testimonials/companies/vacancies; no public metrics ("24/7", placement rate, counts) without a real aggregation contract; no public environment diagnostics — the Laravel/PHP version and `APP_ENV` are never shown on `/`; health stays at `/health/live`, `/health/ready`, `/up` |
