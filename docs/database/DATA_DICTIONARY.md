...
| slug | String | Yes | Unique | Public route identifier for `GET /api/v1/public/companies/{slug}`. Generated once at creation from the company name plus a random discriminator; never regenerated by a later name edit. Carries no authorization meaning (PD-2). | Approved Product Owner decision (PD-2) |
