...
<template>
    ...
    <AppShell persona="recruiter" active="jadwal-seleksi" title="Jadwal Seleksi">
        <h1 class="text-3xl font-bold tracking-tight text-[#002045]">Jadwal Seleksi</h1>
        <p class="mt-2 max-w-2xl text-slate-600">Seluruh jadwal seleksi pada perusahaan Anda. Jadwal baru dibuat dari halaman detail pelamar.</p>
        ...
        <div v-if="!items.length" class="mt-6 rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
            ...
            <p class="mt-2 text-sm text-slate-500">Buat jadwal dari halaman detail pelamar.</p>
        </div>
        ...
        <div v-else class="mt-6 overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                    <tr>
                        <th class="px-4 py-3">Jenis Seleksi</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td class="px-4 py-3 text-right">
                            <Link :href="`/jadwal-seleksi/${item.id}`" class="font-medium text-[#0061a5] hover:underline">Detail</Link>
                            ·
                            <Link :href="`/pelamar/${item.application_id}`" class="font-medium text-[#0061a5] hover:underline">Pelamar</Link>
                        </td>
                    </tr>
                </tbody>
            </table>
